...
def evaluate_all_metrics(
    ...
    skip_mcq=False,overwrite=False
    ):
    ...
    if not os.path.exists(eval_res_filename):
        ...
        existing = pd.read_csv(eval_res_filename)
        already_done = existing[
            (existing['model'] == model) &
            (existing['country'] == country) &
            (existing['language'] == language) &
            (existing['prompt_no'] == prompt_no)
        ]
        if not already_done.empty:
            print(f"[skip] {model} | {country} | {language} | {prompt_no} — already evaluated. Use --overwrite to re-run.")
            return
    ...
    res_df.to_csv(os.path.join(response_dir,os.path.basename(model),f'{os.path.basename(model)}_{country}_{language}_{prompt_no}_response_score.csv'),index=False,encoding='utf-8')

    # Multiple Choice Question
    if language == 'English' and not skip_mcq:
